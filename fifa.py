...
#input parameters
# change exportocsv to True if you want just to export your db to CSV and skip the web scrapping 
justexportocsv = False
...
for key, value in fifa.items():
    ...
    df = pd.read_sql_query('SELECT * FROM FUTHEAD.{};'.format(value), connection)
    df.to_csv(value + '.csv', sep=',', encoding='utf-8', index=False)
    df.to_excel(value + '.xlsx', encoding='utf-8', index=False)
